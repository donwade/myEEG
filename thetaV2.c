...
float thetaV2_coefficients[20] = 
{
// Scaled for floating point

    0.006716449993451663, 0.013432899986903326, 0.006716449993451663, 1.8747984074067225, -0.9077145329188827,// b0, b1, b2, a1, a2
    1, -2, 1, 1.9041917211479398, -0.9254230336064867,// b0, b1, b2, a1, a2
    0.0078125, 0.015625, 0.0078125, 1.9105561642054243, -0.9558802457630298,// b0, b1, b2, a1, a2
    1, -2, 1, 1.9572466078878874, -0.9734236274589974// b0, b1, b2, a1, a2
...
{
    thetaV2Type *result = (thetaV2Type *)malloc( sizeof( thetaV2Type ) ); // Allocate memory for the object
    thetaV2_init( result );                                               // Initialize it
...
{
    thetaV2_executionState executionState;          // The executionState structure holds call data, minimizing stack reads and writes 
    ...
    executionState.pCoefficients = thetaV2_coefficients;    // Each call to filterBiquad() will advance pState and pCoefficients to the next biquad
    ...
    thetaV2_filterBiquad( &executionState );		// Run biquad #0
    executionState.pInput = executionState.pOutput;         // The remaining biquads will now re-use the same output buffer.

    thetaV2_filterBiquad( &executionState );		// Run biquad #1

    thetaV2_filterBiquad( &executionState );		// Run biquad #2

    thetaV2_filterBiquad( &executionState );		// Run biquad #3
